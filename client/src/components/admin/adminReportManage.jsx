...
function AdminReportmanage() {
  ...
  const [reportList,setReportList]=useState([])
    ...
    useEffect(()=>{
      ...
      const modalshow = (id) => {
        reportList.filter((obj) => {
            if (obj._id === id) {
                setModalData({
                  id:obj.postId._id,  
                  desc: obj.postId.desc,
                    img:obj.postId.img,
                    video:obj.postId.video,
                    status:obj.postId.status
                })
                setShowModal(true)
            }
        })
        ...
        <table class="w-full border">
            ...
            <tbody>
                ...
                <tr class="bg-gray-100 text-center border-b text-sm text-gray-600">
                    ...
                    <td class="p-2 border-r">{obj.reportMessage}  (reported by:{obj.userId.name})</td>
                    <td class="p-2 border-r">{obj.userId.name}</td>
                   
                    <td class="p-2 border-r">{obj.postId.desc}</td>
                    <td class="p-2 border-r">{obj.postId.status}</td>
